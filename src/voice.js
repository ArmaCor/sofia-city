...
class Voice {
  ...
  unlockAudioFiles(onDone) {
    ...
    for (const path of paths) {
      ...
      a.currentTime = 0;
      a.volume = vol;
      // Резкий обрыв play() через pause() оставлял этот же элемент (он
      // потом переиспользуется из пула для настоящего проигрывания) в
      // подпорченном состоянии — событие ended у реальной фразы потом
      // не срабатывало (см. NOTES.md). load() принудительно сбрасывает
      // внутренний конвейер элемента в чистое состояние.
      a.load();
